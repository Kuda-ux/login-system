    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage and track company vehicles</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 rounded-2xl p-12 text-center border border-slate-800">
          ...
          <p className="text-slate-500 mb-6">Add your first vehicle to start tracking</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {vehicles.map(vehicle => (
            <div key={vehicle.id} onClick={() => openEdit(vehicle)} className="bg-slate-900 rounded-2xl p-5 border border-slate-800 hover:border-indigo-500/50 cursor-pointer transition group">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-xs px-2 py-1 rounded-full ${vehicle.status === 'active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-400'}`}>
                  {vehicle.status}
                </span>
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 p-4 overflow-y-auto">
          <div className="max-w-lg mx-auto my-8 bg-slate-900 rounded-2xl border border-slate-700 p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <input
                className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:border-transparent"

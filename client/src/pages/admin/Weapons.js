    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Track weapons and clearance assignments</p>
        </div>
        <div className="flex gap-2">
          <button onClick={load} className="p-2.5 bg-slate-800 text-slate-300 rounded-xl hover:bg-slate-700">
            <RefreshCw className="w-5 h-5" />
          </button>
          <button onClick={openNew} className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-medium">
      ...
      {message && (
        <div className="bg-indigo-500/10 border border-indigo-500/30 text-indigo-200 px-4 py-3 rounded-xl">
          {message}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          ...
          <p className="text-slate-500 text-sm">Total Weapons</p>
        </div>
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          <p className="text-2xl font-bold text-amber-400">{weapons.filter(w => w.status === 'issued').length}</p>
          <p className="text-slate-500 text-sm">Issued</p>
        </div>
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
          <p className="text-2xl font-bold text-cyan-400">{clearedGuards.length}</p>
          <p className="text-slate-500 text-sm">Cleared Guards</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 rounded-2xl p-12 text-center border border-slate-800">
          ...
          <p className="text-slate-500 mb-6">Register your first weapon to start tracking</p>
          <button onClick={openNew} className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700">
            <Plus className="w-5 h-5" />
            Register Weapon
          </button>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {weapons.map(weapon => (
            <div key={weapon.id} className="bg-slate-900 rounded-2xl p-5 border border-slate-800 hover:border-indigo-500/50 transition group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${weapon.status === 'issued' ? 'bg-amber-500/20 text-amber-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
                ...
                <span className={`text-xs px-2 py-1 rounded-full ${weapon.status === 'available' ? 'bg-emerald-500/20 text-emerald-400' : weapon.status === 'issued' ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-700 text-slate-400'}`}>
                  {weapon.status}
                </span>
              </div>
              
              <p className="text-sm text-slate-500 mb-2">{weapon.make} {weapon.model} {weapon.caliber && `(${weapon.caliber})`}</p>
              ...
              <div className="flex gap-2 mt-4">
                <button onClick={() => openEdit(weapon)} className="flex-1 py-2 bg-slate-800 text-slate-300 rounded-lg text-sm hover:bg-slate-700">
                  Edit
                </button>
                {weapon.status === 'available' ? (
                  <button onClick={() => openIssue(weapon)} className="flex-1 py-2 bg-amber-600 text-white rounded-lg text-sm hover:bg-amber-700 flex items-center justify-center gap-1">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/70 p-4 overflow-y-auto">
          <div className="max-w-lg mx-auto my-8 bg-slate-900 rounded-2xl border border-slate-700 p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <input
                  className="px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500"
                  placeholder="Make"
                  value={form.make}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 p-4 overflow-y-auto">
          <div className="max-w-lg mx-auto my-8 bg-slate-900 rounded-2xl border border-slate-700 p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowIssueModal(false)}><X className="text-slate-400" /></button>
            </div>
            
            <div className="bg-slate-800 rounded-xl p-4 mb-6">
              <p className="text-white font-semibold">{selectedWeapon.serial_number}</p>
              <p className="text-slate-400 text-sm">{selectedWeapon.weapon_type} - {selectedWeapon.make} {selectedWeapon.model}</p>
            </div>
            ...
            {clearedGuards.length === 0 ? (
              ...
              <form onSubmit={handleIssue} className="space-y-4">
                ...
                />
                <button type="submit" className="w-full py-3 bg-amber-600 text-white rounded-xl font-semibold hover:bg-amber-700">
                  Issue Weapon
                </button>
              </form>
            )}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 p-4 overflow-y-auto">
          <div className="max-w-lg mx-auto my-8 bg-slate-900 rounded-2xl border border-slate-700 p-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowReturnModal(false)}><X className="text-slate-400" /></button>
            </div>
            
            <div className="bg-slate-800 rounded-xl p-4 mb-6">
              <p className="text-white font-semibold">{selectedWeapon.serial_number}</p>
              <p className="text-slate-400 text-sm">{selectedWeapon.weapon_type} - {selectedWeapon.make} {selectedWeapon.model}</p>
              <p className="text-amber-400 text-sm mt-2">Currently held by: {selectedWeapon.holder_name}</p>
            </div>

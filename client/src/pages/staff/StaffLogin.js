...
function StaffLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        if (['staff', 'admin'].includes(result.user.role)) {
          navigate('/staff');
        } else {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-[#d4ae2a]/5 rounded-full blur-[150px]" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[#d4ae2a] text-sm font-medium uppercase tracking-[0.15em]">Guard Attendance & Patrols</p>
        </div>

        {/* Login Form */}
        <div className="bg-[#111111] border border-[#1f1f1f] rounded-3xl p-8 shadow-2xl shadow-black/50">
          <h2 className="text-xl font-semibold text-white mb-1">Guard Sign In</h2>
          <p className="text-[#555] text-sm mb-7">Clock in, track patrols, and manage your shift</p>
        ...
        <p className="text-center text-[#444] text-xs mt-8 tracking-wide">
          FOR AUTHORIZED GUARDS ONLY &middot; CONTACT HQ FOR ACCESS
        </p>
      </div>
    </div>
  );
